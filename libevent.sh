...
if [ ! -d "${SRCDIR}/${LIBNAME}-${VERSION}-stable" ]; then
if [ ! -e "${LIBNAME}-${VERSION}-stable.tar.gz" ]; then
echo "download ${LIBNAME} from here https://github.com/nmathewson/Libevent/archive/release-${VERSION}-stable.tar.gz"
curl -OL -v "https://github.com/nmathewson/Libevent/archive/release-${VERSION}-stable.tar.gz"
fi
